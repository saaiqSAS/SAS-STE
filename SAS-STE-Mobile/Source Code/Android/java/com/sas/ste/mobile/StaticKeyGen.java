...
public class StaticKeyGen extends AppCompatActivity {
    ...
    private void selectStaticKeysFile() {
        Toast.makeText(StaticKeyGen.this, "This Feature Is Not Available Yet. Please Enter Existing StaticKeys.stkey File Path Manually", Toast.LENGTH_SHORT).show();
    }

    private void restoreDefaultStaticKeysFile() {
        defaultStaticKeysFileGen();
        Toast.makeText(StaticKeyGen.this, "Default StaticKeys.stkey File Restored", Toast.LENGTH_SHORT).show();
    }
    ...
    protected static void defaultStaticKeysFileGen() {
        try {
            File staticKeysFile = new File(Environment.getExternalStorageDirectory() + "/SAS-STE/.ProgramFiles/StaticKeys.stkey");
            if (staticKeysFile.exists()) {
                staticKeysFile.delete();
            }

            FileWriter defaultStaticKeysFile = new FileWriter(Environment.getExternalStorageDirectory() + "/SAS-STE/.ProgramFiles/StaticKeys.stkey", true);
            ...
            if (staticKeysFilePath.equals("")) {
                ...
                if (!exitStaticKeysFile.exists()) {
                    ...
                    while (fileHasNextLine) {
                        StaticKeys += scannedExistStaticKeysFile.nextLine() + "\n";
                        fileHasNextLine = scannedExistStaticKeysFile.hasNextLine();
                    }
                    viewStaticKeys();
                }
